...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Comece com o código do país do seu número do WhatsApp, exemplo : +554495999900")))

            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Digite seu número do WhatsApp 😍\nPor exemplo: +554495999900 : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ Jvps_Mods_TechMod ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: joaovctor20022004`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: jvps2004 `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @joaovitorp2004 `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: ${owner}`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: Jvps_Mods_TechMod\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
